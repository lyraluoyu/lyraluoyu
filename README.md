# Hi there, I'm Luo Yu. 👋
I am a Junior student at **Shanghai Jiao Tong University**.

### Research & Projects
- **DFTAgent:** Developing an LLM-driven optimization framework for Density Functional Theory.
- **3D-Printing Software Interface:** Designed interface for 3D-Printing software.

### Awards & Honors
- Meritorious Award (First Prize) in the 2024 Interdisciplinary Contest in Modeling
- Dean's Scholarship & First Prize Academic Scholarship

### Skills & Interests
- **Languages:** Python, C++, C
- **Open Source Interests:** Interested in niche Python-based scientific tools. Passionate about **AI for Science (AI4S)**, **LLM Agents**, and **Quantitative Computing**. 
